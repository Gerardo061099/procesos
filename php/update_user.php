...
if ( $pass != '' ) {
    ...
    $query = mysqli_query( $conexion, "UPDATE $tb_users SET nombre = '$nombre', apellidos = '$apellidos', user = '$mail', pass = '$pass_encrypt', numero_empleado = '$n_control', update_at = now() WHERE id = '$id'" );
    if ( $query == true ) {
        if ( $estado == 'Activo' ) {
            $estado_a = '1';
        } else {
    ...
    $query = mysqli_query( $conexion, "UPDATE $tb_users SET nombre = '$nombre', apellidos = '$apellidos', user = '$mail', numero_empleado = '$n_control', update_at = now() WHERE id = '$id'" );
    if ( $query == true ) {
        if ( $estado == 'Activo' ) {
            $estado_a = '1';
        } else {
